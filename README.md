...
gzip-compressed POSIX tarball — without touching the disk in between. This is
ideal for eating large text logs (e.g. requests containing json) scattered over
many files.

Despite the name, nothing gets mounted by the host OS. TarFS is a library-level
view of an archive, not a FUSE filesystem: it gives your Julia code
`readfile`/`writefile` semantics over a single `.tar.gz` on disk.
